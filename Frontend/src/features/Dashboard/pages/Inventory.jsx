    ...
    <div className="pt-8 pb-20 px-6 lg:px-12 bg-[#FBF9F6]">
      ...
      <header className="max-w-[1440px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-16">
        <div className="space-y-2">
          ...
          <h1 className="text-4xl md:text-5xl font-serif text-[#1b1c1a]">
            Inventory Management
          </h1>
          <p className="text-sm text-[#7a6e63]">
            Overview of your curated pieces and collection metrics.
          </p>
        </div>
      ...
      <div className="max-w-[1440px] mx-auto flex flex-col md:flex-row gap-8 items-center justify-between mb-12 pb-8 border-b border-[#e8e2da]">
        <div className="relative w-full md:w-[450px] group">
          <Search
            size={16}
            className="absolute left-0 top-1/2 -translate-y-1/2 text-[#7a6e63] group-focus-within:text-[#C9A96E] transition-colors"
          />
          <input
            type="text"
        ...
        {productLoading && sellerProducts.length === 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-10 gap-y-16">
            {Array.from({ length: 8 }).map((_, idx) => (
              <div key={idx} className="animate-pulse space-y-5">
                <div className="aspect-[4/5] rounded-2xl bg-[#efe9e1]" />
                <div className="h-7 w-3/4 rounded bg-[#efe9e1]" />
                <div className="h-4 w-1/2 rounded bg-[#efe9e1]" />
                <div className="h-4 w-full rounded bg-[#efe9e1]" />
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-10 gap-y-16">
            {filteredProducts.map((product) => (
              ...
              >
                {/* Image Container */}
                <div
                  className="aspect-[4/5] max-h-[450px] overflow-hidden mb-6 relative"
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <p className="text-[12px] line-clamp-2 leading-relaxed font-medium mt-1 uppercase tracking-wider text-black opacity-30 italic">
          ...
          <div className="py-32 text-center flex flex-col items-center animate-in zoom-in-95 duration-1000">
            <span
              className="text-[10px] uppercase tracking-[0.4em] font-black mb-6"
              style={{ color: "#C9A96E" }}
            >
              Empty Vault
            </span>
            <p
              className="max-w-md mx-auto text-2xl leading-relaxed italic"
              style={{
                fontFamily: "'Cormorant Garamond', serif",
                color: "#7A6E63",
              }}
            >
              ...
              <span className="text-sm font-sans uppercase tracking-[0.2em] not-italic text-[#1b1c1a]/30 mt-4 block">
                Begin by creating a new listing.
              </span>
            </p>
            <button
              onClick={() => navigate("/seller/create-product")}
