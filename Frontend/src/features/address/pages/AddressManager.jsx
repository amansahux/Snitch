...
const optionalTextField = (label, min = 3, max = 100) =>
  ...
  }, z.string().min(min, `${label} must be at least ${min} characters`).max(max, `${label} cannot exceed ${max} characters`).optional());

const optionalMobileField = z.preprocess((value) => {
  if (typeof value !== "string") return value;
  const trimmed = value.trim();
  return trimmed === "" ? undefined : trimmed;
}, z.string().regex(/^[0-9]{10}$/, "Alternate mobile must be 10 digits").optional());
