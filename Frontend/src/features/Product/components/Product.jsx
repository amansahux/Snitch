...
const Product = ({ product }) => {
  const price = product.price?.selling
    ? (product.price.currency === "INR" ? `₹${product.price.selling.toLocaleString()}` : `$${product.price.selling.toLocaleString()}`)
    : (product.price?.amount ? (product.price.currency === "INR" ? `₹${product.price.amount.toLocaleString()}` : `$${product.price.amount.toLocaleString()}`) : "N/A");

  const mrp = product.price?.mrp
    ? (product.price.currency === "INR" ? `₹${product.price.mrp.toLocaleString()}` : `$${product.price.mrp.toLocaleString()}`)
    : null;
      ...
      >
        ...
        <div className="absolute bottom-6 left-6 right-6 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-700">
          <div className="bg-white/90 backdrop-blur-md py-4 rounded-xl flex items-center justify-center gap-2 shadow-luxury">
            <span className="text-[9px] font-black uppercase tracking-[0.3em] font-sans text-[#1b1c1a]">
              View Perspective
            </span>
      ...
      <div className="pt-8 space-y-3 px-2">
        <div className="flex justify-between items-start gap-4">
          ...
          <div className="flex flex-col items-end ">
            ...
            <span className="text-sm font-inter line-through text-[#7a6e63] mt-1 shrink-0">
              {mrp}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-t border-[#e8e2da] pt-3">
          ...
          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#7a6e63]/40">
            Authenticated Piece
          </p>
        </div>
      </div>
