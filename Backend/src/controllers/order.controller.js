    ...
    if (req.user?.email) {
      const orderItemsHtml = order.items
        ...
        <div style="display: flex; align-items: center; padding: 20px 0; border-bottom: 1px solid #F3EEE8;">
          <img src="${item.variant?.images?.[0]?.url || "https://placehold.co/100x130?text=Product"}" alt="${item.product?.title || "Product"}" style="width: 70px; height: 95px; object-fit: cover; border-radius: 12px; margin-right: 25px; border: 1px solid #F3EEE8;">
          <div style="flex: 1;">
            <h4 style="margin: 0; font-family: 'Playfair Display', serif; font-size: 15px; color: #1B1C1A; font-weight: 700;">${item.product?.title || "Product"}</h4>
            <p style="margin: 6px 0 0; font-family: 'Inter', sans-serif; font-size: 11px; color: #7A6E63; text-transform: uppercase; letter-spacing: 1px;">Size: ${item.size} | Qty: ${item.quantity}</p>
            <p style="margin: 8px 0 0; font-family: 'Playfair Display', serif; font-size: 15px; font-weight: 700; color: #1B1C1A;">₹${item.discountedPrice?.toLocaleString("en-IN") || "0"}</p>
          </div>
        </div>
      `,
        )
      ...
      const subject = `The Snitch Manifest: Order #SN-${razorpay_order_id.slice(-6).toUpperCase()} Confirmed`;
                ...
                <table width="600" border="0" cellspacing="0" cellpadding="0" style="background-color: #ffffff; border-radius: 40px; overflow: hidden; box-shadow: 0 40px 80px rgba(27,28,26,0.05);">
                  ...
                  <tr>
                    <td align="center" style="padding: 50px 0; border-bottom: 1px solid #F3EEE8;">
                      ...
                      <div style="width: 40px; height: 1px; background-color: #C9A96E; margin: 15px auto;"></div>
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            <td>
                              ...
                              <p style="margin: 8px 0 0; font-family: 'Playfair Display', serif; font-size: 18px; font-weight: 700; color: #1B1C1A;">#SN-${razorpay_order_id.slice(-8).toUpperCase()}</p>
                            </td>
                            <td align="right">
                              ...
                              <p style="margin: 8px 0 0; font-family: 'Playfair Display', serif; font-size: 18px; font-weight: 700; color: #C9A96E;">₹${order.totalAmount?.toLocaleString("en-IN") || "0"}</p>
                            </td>
                          </tr>
                        </table>
                      </div>
      ...
      `;
      sendEmail(req.user.email, subject, "", html).catch((err) =>
        console.error("Email send failed:", err),
      );
    }
  } catch (err) {
    console.error("Failed to generate order confirmation email:", err);
  }
